...
**Tool-calling.** Qwen2.5 on local MAX returns Hermes-style `<tool_call>…</tool_call>` in response text instead of OpenAI `tool_calls`. When the served model ID matches Qwen 2.5, [`scripts/qwen25-max-tool-call-proxy.py`](scripts/qwen25-max-tool-call-proxy.py) starts automatically on port 8001 (MAX API stays on 8000; Prometheus metrics move to 9090 so they do not collide). The proxy promotes Hermes blocks and bare JSON tool objects (including nested `arguments`) into structured `tool_calls`, defaults `task_done` to `{"state":"DONE"}`, rewrites literal `<current_file_path>` tool args to the real path from the OCR prompt, and strips leading `/` from repo paths (so `/scripts/foo.sh` becomes `scripts/foo.sh`). External `llm_url` endpoints skip the proxy. The job fails (and does not post a false LGTM) when the session has `review_item_failed`, no successful `review_item_done`, or tool errors with zero comments.
